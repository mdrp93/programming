...
public class AutomataCelularSeeds extends JPanel { 
    private boolean[][] grid;
    private int size;
    private int cellSize;
    private int delay;

    public AutomataCelularSeeds(int size, int cellSize, int delay) {
        this.size = size;
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Definir colores para las células vivas y muertas
        Color colorViva = Color.WHITE;
        Color colorMuerta =Color.BLACK; // Gris oscuro
    
        // Dibujar las células en dos colores diferentes
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (grid[i][j]) {
                    ...
                    // Dibujar células muertas en gris oscuro
    ...
    public static void main(String[] args) {
        int size = 100; // Tamaño del grid
        int cellSize = 10; // Tamaño de cada célula en píxeles
        int delay = 50; // Retardo en milisegundos entre generaciones

        AutomataCelularSeeds automata = new AutomataCelularSeeds(size, cellSize, delay);
